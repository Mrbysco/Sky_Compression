...
import hqbanana.skycompression.base.tile.TileItemInventory;
...
public class ContainerBase extends Container {
	...
	public ItemStack transferStackInSlot(EntityPlayer playerIn, int fromSlot) {
		...
		if (slot != null && slot.getHasStack()) {
			...
			if (fromSlot < 36) {
				if (!this.mergeItemStack(current, 36, slotCount, false)) return ItemStack.EMPTY;
			} else {
				if (!this.mergeItemStack(current, 0, 36, false)) return ItemStack.EMPTY;
			}
